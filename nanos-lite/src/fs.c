...
size_t fs_read(int fd, void *buf, size_t len) {
  if (fd >= 0 && fd <= SPECIAL_FILE_NUM) {
    size_t i = file_table[fd].read(buf, file_state[fd].open_offset, len);
...
size_t fs_write(int fd, const void *buf, size_t len) {
  if (fd >= 0 && fd <= SPECIAL_FILE_NUM) {
    size_t i = file_table[fd].write(buf, file_state[fd].open_offset, len);
    file_state[fd].open_offset += len;
    return i;
  }
  int check_flag = check_open_overflow(fd, len);
...
size_t fs_lseek(int fd, size_t offset, int whence) { 
  ...
  if (!file_state[fd].is_open || file_state[fd].open_offset > file_table[fd].size) { 
    ...
    if (file_state[fd].is_open) printf("Panic: fd: %d, off: %d, when: %d, openoff: %d, sz: %d\n", fd, offset, whence, file_state[fd].open_offset, file_table[fd].size);
