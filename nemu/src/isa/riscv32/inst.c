...
#ifdef CONFIG_FTRACE
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc + 4, s->dnpc = imm + src1, s->jmpAddr = s->dnpc);
#else
...
#ifdef CONFIG_FTRACE
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4, s->dnpc = s->pc + imm, s->jmpAddr = s->dnpc);
#else
...
#ifdef CONFIG_FTRACE
  s->Type = (s->isa.inst.val == 0x00008067) ? 1 : 0;
#endif
